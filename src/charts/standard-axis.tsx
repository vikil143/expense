...
import { padding } from './dummy-data';
...
export default function StandardAxis<T>({ xScale, yScale, children, height, width, xTick = 5, yTick = 5, showHLine = false }: StandardAxisProps<T>) {
        ...
        {xTicks.map((tick, i) => {
          ...
          <G key={i} x={x} y={0}>
            ...
            <Text
              ...
              textAnchor="middle"
            >
              {moment(tick).format('dddd').substring(0, 3)}
            </Text>
          </G>
        )})}
      </G>
